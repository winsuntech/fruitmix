...
describe("test files", function() {
  beforeEach(function(done){
    User.remove({}, function(err){
      if (err) throw err;
      var userObj = { 
        "uuid" : "35fd79b7-a4fe-4be5-a525-c5faf9749bd4",
        "username" : "admin", 
        "password" : "123456", 
  ...
  after(function(done){
    spawn('bash', ['/trynode/john.sh','/mnt']).done();
  });

  it("GET /files should return 404 if uuid is invalid", function(done) {
    request(app)
      .get('/files/xx')
      .set('Authorization','JWT eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1dWlkIjoiMzVmZDc5YjctYTRmZS00YmU1LWE1MjUtYzVmYWY5NzQ5YmQ0In0.gQesS8VegE0zEgGS-pbgorPEKd9vU60p0MG0veeS98k')
      .expect(404, done);
  });
